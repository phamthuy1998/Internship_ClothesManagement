...
namespace ClothesManament.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        [AcceptVerbs("POST")]
        [HttpPost]
        public ResponseObjectModel<SPGetAccountInfoUserIDEmployee_Result> adminLogin([FromBody] LoginParam loginParam)
        {
            var result = entities.SP_Login_Admin(loginParam.email, loginParam.password).FirstOrDefault();
            if (result.HasValue)
            {
                int resultInt = result.Value;
                if (resultInt == -1)
                {
                    return new ResponseObjectModel<SPGetAccountInfoUserIDEmployee_Result>()
                    {
                        message = "Username không tồn tại",
                        status = false,
                        code = 200,
                ...
                {
                    var accountInfo = entities.SPGetAccountInfoUserIDEmployee(resultInt).FirstOrDefault(); ;
