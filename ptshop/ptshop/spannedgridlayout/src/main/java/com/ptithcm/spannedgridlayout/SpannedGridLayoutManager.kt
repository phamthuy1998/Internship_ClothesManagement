...
open class SpannedGridLayoutManager(val orientation: Orientation,
    ...
    private val rectangleHeight = 200
    /**
    ...
    override fun onLayoutChildren(recycler: RecyclerView.Recycler, state: RecyclerView.State) {
        ...
        layoutEnd = if (scroll != 0) {
            val currentRow = (scroll - layoutStart) / (rectsHelper.itemSize + rectangleHeight)
            currentRow * (rectsHelper.itemSize + rectangleHeight)
        } else {
        ...
        if (itemCount != 0 && pendingScrollToPosition != null && pendingScrollToPosition >= spans) {
            ...
            if (currentRow != null) {
                scroll = getPaddingStartForOrientation() + (currentRow * (rectsHelper.itemSize + rectangleHeight))
            }
        ...
        val overScroll = scroll +rectangleHeight + size - layoutEnd - getPaddingEndForOrientation()
    ...
    protected open fun measureChild(position: Int, view: View) {
        ...
        val itemHeight = freeRectsHelper.itemSize + rectangleHeight
    ...
    protected open fun updateEdgesWithNewChild(view: View) {
        ...
        val newLayoutEnd = childStart + rectsHelper.itemSize + rectangleHeight
    ...
    protected open fun scrollBy(delta: Int, recycler: RecyclerView.Recycler, state: RecyclerView.State): Int {
        ...
        val canScrollForward = (firstVisiblePosition + childCount) <= state.itemCount &&
                (scroll + size) < (layoutEnd + rectsHelper.itemSize + rectangleHeight + getPaddingEndForOrientation())
        delta > 0
    ...
    protected open fun scrollBy(distance: Int, state: RecyclerView.State): Int {
        ...
        val start = 0
        val end = layoutEnd + rectsHelper.itemSize + paddingEndLayout + rectangleHeight
    ...
    protected open fun fillBefore(recycler: RecyclerView.Recycler) {
        val currentRow = (scroll - getPaddingStartForOrientation()) /( rectsHelper.itemSize + rectangleHeight)
        val lastRow = (scroll + size - getPaddingStartForOrientation()) / ( rectsHelper.itemSize + rectangleHeight)
    ...
    protected open fun fillAfter(recycler: RecyclerView.Recycler) {
        ...
        val lastAddedRow = layoutEnd / ( rectsHelper.itemSize + rectangleHeight)
        val lastVisibleRow =  visibleEnd / ( rectsHelper.itemSize + rectangleHeight)
...
open class RectsHelper(val layoutManager: SpannedGridLayoutManager,
    ...
    val start: Int get() {
        return if (orientation == SpannedGridLayoutManager.Orientation.VERTICAL) {
            freeRects[0].top * itemSize + 200
        } else {
            freeRects[0].left * itemSize + 200
        }
    }
    ...
    val end: Int get() {
        return if (orientation == SpannedGridLayoutManager.Orientation.VERTICAL) {
            (freeRects.last().top + 1) * itemSize + 200
        } else {
            (freeRects.last().left + 1) * itemSize + 200
        }
    }
